...
function nixos_anywhere() {
	...
	SHELL=/bin/sh nix run github:nix-community/nixos-anywhere/69ad3f4a50cfb711048f54013404762c9a8e201e -- --ssh-port "$ssh_port" --extra-files "$temp" --flake .#"$target_hostname" root@"$target_destination"
...
function update_sops_file() {
	...
	cd "${git_root}"/../hyperionix-secrets
	...
	green "Updating hyperionix-secrets/.sops.yaml"
	cd -
}
...
function generate_user_age_key() {
	...
	secret_file="${git_root}"/../hyperionix-secrets/secrets.yaml
...
if [[ $updated_age_keys == 1 ]]; then
	...
	nix flake lock --update-input hyperionix-secrets
fi
...
if yes_or_no "Do you want to copy your full nix-config and hyperionix-secrets to $target_hostname?"; then
	green "Adding ssh host fingerprint at $target_destination to ~/.ssh/known_hosts"
	ssh-keyscan -p "$ssh_port" "$target_destination" >>~/.ssh/known_hosts || true
	green "Copying full nix-config to $target_hostname"
	sync "$target_user" "${git_root}"/../hyperionix
	green "Copying full hyperionix-secrets to $target_hostname"
	sync "$target_user" "${git_root}"/../hyperionix-secrets
...
if yes_or_no "Do you want to rebuild immediately?"; then
	...
	$ssh_cmd -oForwardAgent=yes "cd hyperionix && just rebuild"
fi
else
	echo
	green "NixOS was successfully installed!"
	echo "Post-install config build instructions:"
	echo "To copy nix-config from this machine to the $target_hostname, run the following command from ~/hyperionix"
	echo "just sync $target_user $target_destination"
	echo "To rebuild, sign into $target_hostname and run the following command from ~/hyperionix"
	echo "cd hyperionix"
	echo "just rebuild"
	echo
fi
